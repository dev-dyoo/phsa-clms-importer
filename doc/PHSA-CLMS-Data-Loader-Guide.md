...
| **PowerShell scripts** (steps 10, 11) | Windows PowerShell 5.0+ (`#Requires -Version 5.0`). They walk a Windows network file share (`\\phsabc\root\...`) so they must run on a Windows host that can see the share. Depend on a `_common/OracleFusionCommon.psm1` module **that is not in this repo** — they were lifted from the `phsa-clms-ps` project; port or restore that module before running them here. |
...
2. **Deps:** Windows PowerShell 5.0+, the `_common/OracleFusionCommon.psm1` module (not in this repo — see §1), `<step>/.settings`, and read access to the contract folders on the `\\phsabc\root\...` share.
...
2. **Deps:** Windows PowerShell 5.0+, `_common/OracleFusionCommon.psm1` (not in repo), `<step>/.settings`, and read access to the document files on the share (it reads the bytes and base64-encodes them).
...
- **The PowerShell steps (10, 11) need `_common/OracleFusionCommon.psm1`**, which is not in this repo. Restore/port it (it provides `Read-ScriptSettings`, `Get-BasicAuthHeader`, `Invoke-ApiWithRetry`, `New-TimestampedLogPath`/`...CsvPath`, `Write-ScriptLog`, `Test-ApiSanityCheck`, `Get-DebugModeChoice`).
